...
namespace ConsoleApp7
{
    class Program
    {
        ...
        //}
            ...
            {
                //int index2 = moreThings.IndexOf(stuff);
                //Console.WriteLine(stuff + index2);
                // For some reason, two identical entries in a list have the same index even though they show up twice when I iterate through the list?

                if (string.Equals(stuff, searchTerm2, StringComparison.CurrentCultureIgnoreCase))
                {
                    int index2 = moreThings.IndexOf(searchTerm2);
                    Console.WriteLine("You searched for " + searchTerm2 + ", which is at the index " + index2 + " of this list!");
                    found2 = 2;
                }
            }
            ...
            foreach (string list in AnotherList)
            {
                ...
                {
                    index3 = AnotherList.IndexOf(search2);

                    if (index3 == indexdup)
                    {
                        Console.WriteLine("Duplicate Entry! " + search2 + " has already appeared in the list at index " + index3 + "!");
                    }
                    else
                        Console.WriteLine("You searched for " + search2 + ", which is at the index " + index3 + " of this list!");
                        found3 = 2;
                        indexdup = index3;
